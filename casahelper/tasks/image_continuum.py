...
def image_continuum(data, robust=[-1,0.5,2], nsigma=3.0, fits=False, \
        ...
        parallel=False, gain=0.1, scales=[0], cyclefactor=1, \
        cycleniter=-1):
    ...
    for robust_value in robust:
        tclean(vis=[track.ms for track in tracks], spw=[track.spw for \
                ...
                tracks], imagename=combined.image+suffix.\
                format(robust_value), specmode='mfs', nterms=nterms, \
                niter=combined.niter, gain=gain, nsigma=nsigma, \
                imsize=combined.imsize, cell=combined.cell, stokes='I', \
                deconvolver=deconvolver, gridder='standard', \
                ...
                scales=scales, parallel=parallel, cyclefactor=cyclefactor, \
        ...
        if savemodel:
            ...
            if not model_empty:
                tclean(vis=[track.ms for track in tracks], spw=[track.spw \
                        for track in tracks], field=[track.science for \
                        track in tracks], imagename=combined.image+\
                        suffix.format(robust_value), specmode='mfs', \
                        nterms=nterms, niter=0, gain=gain, nsigma=nsigma, \
                        imsize=combined.imsize, cell=combined.cell, \
                        stokes='I', deconvolver=deconvolver, \
                        ...
                        savemodel="modelcolumn", calcres=False, \
                        calcpsf=False, fastnoise=False, verbose=True, \
                        pblimit=-0.2, uvtaper=uvtaper, scales=scales, \
                        parallel=parallel, cyclefactor=cyclefactor, \
                        cycleniter=cycleniter)

        # Export the relevant images to fits files if requested.

        if fits:
            exportfits(imagename=combined.image+suffix.\
                    format(robust_value)+".image"+terms_suffix, \
                    fitsimage=combined.image+\
